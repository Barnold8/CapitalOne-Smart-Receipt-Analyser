...
export default {
  ...
  data() {
    ...
    const startDate = today.clone().subtract(31, 'days').startOf('day');
    return {
      startDate,
      disableNext: true,
      selectedDate: null,
  ...
  methods: {
    prev() {
      this.startDate = moment(this.startDate).subtract(31, 'days').startOf('day');
      this.endDate = moment(this.endDate).subtract(31, 'days').endOf('day');
    ...
    next() {
      ...
      if (!this.disableNext) {
        this.startDate = moment(this.startDate).add(31, 'days').startOf('day');
        this.endDate = moment(this.endDate).add(31, 'days').endOf('day');
